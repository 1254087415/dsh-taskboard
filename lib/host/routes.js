	...
	const handler = async (req, res) => {
		try {
			...
			if (req.method === "GET") {
				...
				if (pathname === `/dsh-taskboard/sessions/candidates`) {
					try {
						const [projRaw, wsRaw] = await Promise.all([readFile(dshHomePath("storages", "session_projcache.json"), "utf8"), readFile(dshHomePath("storages", "workspace.json"), "utf8")]);
						const proj = JSON.parse(projRaw);
						const wsDoc = JSON.parse(wsRaw);
						const archived = new Set(wsDoc.global?.archivedSessionIds ?? []);
						const byPath = /* @__PURE__ */ new Map();
						for (const [wid, w] of Object.entries(wsDoc.tables?.workspaces ?? {})) byPath.set(w.path, {
							id: wid,
							title: w.title ?? wid
						});
						const NOISE = [
						...
						for (const [sid, s] of Object.entries(proj.tables?.sessions ?? {})) {
							...
							if (typeof subVal === "object" && subVal !== null && subVal.identity?.mode !== void 0) continue;
							...
							candidates.push({
								...
								taskLike: TASKLIKERE.test(title)
							});
						}
						json(res, {
							ok: true,
				...
				if (pathname === `/dsh-taskboard/sessions/links`) {
					try {
						...
						for (const [sessionId, taskId] of Object.entries(links)) {
							...
							if (task === void 0 || task.trashedAt !== void 0) continue;
							rows.push({
								sessionId,
								taskId,
								title: task.title,
			...
			if (pathname === `/dsh-taskboard/sessions/import`) {
				try {
					const picksRaw = body.picks;
					if (!Array.isArray(picksRaw) || picksRaw.length === 0) throw new Error("Error: invalid_input: picks must be a non-empty array");
					const created = [];
					const errors = [];
					for (const pick of picksRaw) try {
