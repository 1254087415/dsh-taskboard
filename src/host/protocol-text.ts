...
export const TASKBOARD_PROTOCOL = [
  ...
  '9. 开场自检（每个新会话）：用户请求/讨论的内容若属「可自动化交付的任务」（代码开发、技术调研、',
  '   写作/文档生成、数据整理、批量处理、剪藏精读总结等有客观交付物、可机器验证），**自动建卡**：',
  '   用 taskboard_create 建为待办 todo（status="todo" 即默认值，urgency 按重要度，description 记会话上下文），',
  '   并在回复里告知「已自动建卡 t-xxxxxx（待办），确认后我可认领执行」；建卡后不自动认领、不执行。',
  '   「待规划 backlog」只用于用户明说不急、或属发散性后续需求的场景；没有明示就是默认 todo。',
  '   闲聊、问答、生活杂务、需要用户拍板的决策不建卡；会话已属于某看板任务执行时（session-taskboard-*）不重复建。',
  '10. 留存会话可导入看板：用户想把历史/留存会话加入看板时，按 taskboard skill 的「会话导入」节：',
  '    读 ~/.dsh/storages/session_projcache.json 列候选（标题+项目），用户挑选后批量 taskboard_create。',
